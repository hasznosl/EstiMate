...
export class GlobalProvider extends React.Component<{}, IState> {
  ...
  resetDataState = async () => {
    ...
    const virtualSpending =
      virtualSpendings[Object.keys(virtualSpendings).length - 1];
    const financialGoal =
      financialGoals[Object.keys(financialGoals).length - 1];
    const currencyNames = Object.keys(realmCurrencies).map(
      key => realmCurrencies[key].name
    );
  ...
  onClickSaveManuallyImportedData = async ({
    beginningDate,
    account,
    endDate,
    amount,
    ...
    const increment = Math.floor(
      parseInt(amount) / differenceInDays(endDate, beginningDate)
    );
    ...
    let dbAccount = dbAccounts.find(
      dbAcc => dbAcc.name === accountName && dbAcc.currency === currency
    );

...
import { chartContainer, generalStyles, yAxis, backgroundLineColor, lineColor } from '../styles';
...
const TotalAveragePerDayOverTime = () => {
	...
	const { netWorthOverTimeToFuture, birthDay } = useContext<IContextType>(GlobalContext);
	...
	const xyData = relevantDates.filter(isLastDayOfMonth).map((date) => ({
		x: new Date(date),
		y: dateValueMap[date]
	}));
								...
								return xCoord < 10 ? null : (
									<G key={dat.toString()}>
										...
										<Text x={yAxis.innerMargin + 1} fontSize="8" y={yCoord - 1}>
											{Math.floor(dateValueMap[formatDate(dat)] * 10) / 10}
										</Text>
										{/* horizontal lines */}
										<Line
											x1={0}

...
import {
  ...
  NetWorthOverTimeType,
  ImportantDateType,
} from './types';
import persistLinesAsTransactions, {
  persistJsonAsTransactions,
} from '../persistenceUtils/persistLinesAsTransactions';
...
export const getN26Transactions = async () => {
  let transactions = [];
  ...
  accounts.forEach((account, index) => {
    console.log('account one', account.name, index);
    account.n26Transactions.forEach((transaction) =>
  ...
  return transactions;
  /*const asdf = transactions.reduce((acc, transaction) => {
		return transaction.category === 'Travel & Holidays' ? [...acc, transaction] : acc
	}, [])

	console.log('transactions', transactions.length)

	const byMonth = asdf.reduce((acc, transaction) => {
		const monthYear = transaction.date.split('-')
		return acc[`${monthYear[0]}/${monthYear[1]}`] ?
		{ ...acc, [`${monthYear[0]}/${monthYear[1]}`]: parseFloat(transaction.amount) + acc[`${monthYear[0]}/${monthYear[1]}`]} :
		{ ...acc, [`${monthYear[0]}/${monthYear[1]}`]: parseFloat(transaction.amount) }
	}, {})
...
export const populateFromJson = async ({callback}) => {
  ...
  await persistJsonAsTransactions({
    rows: myjson.rows,
    currency: {
      name: 'EUR',
      exchangeToDefault: '1',
...
export const calculateCurrentWorthOfDeterioratingItem = (account) => {
  const transactions = orderBy(account.transactions, (transaction) =>
    new Date(transaction.date).getTime(),
  );
  ...
  return Math.floor(
    getInitialValueOfItem(account) /
      (account.deteriorationConstant * daysSince + 1),
  );
};
...
export const calculateProjectedNetWorthOverTime = ({
  ...
  let numOfDays = differenceInDays(
    endOfMonth(lastDate),
    importantDates[importantDates.length - 1] || dates[0],
  );
  const xDaysBefore = formatDate(subDays(endOfMonth(lastDate), numOfDays));
  const levelAtEndOfThisMonth =
    netWorthOverTime[endOfPrevMonth] + projectedSavingForThisMonth;
  const increment =
    levelAtEndOfThisMonth - netWorthOverTime[xDaysBefore] ||
    levelAtEndOfThisMonth;
    ...
    while (isBefore(currentDate, addDays(financialGoal.date, 1))) {
      if (!netWorthOverTime[currentDate] && isBefore(currentDate, new Date())) {
        ...
        projectedNetWorthOverTime[currentDate] =
          netWorthOverTime[lastDate] + i * artificialShortTermGrowthRate;
      } else {
        const endOfThisMonth = formatDate(endOfMonth(new Date()));

        projectedNetWorthOverTime[currentDate] =
          (projectedNetWorthOverTime[endOfThisMonth] ||
            netWorthOverTime[endOfThisMonth] ||
            lastValue) +
          (i - differenceInDays(addDays(endOfMonth(lastDate), 1), new Date())) *
            longTermGrowthRate;
      }
...
export const calculatePeriodsAveragePerDayOverTimeData = ({
  ...
  while (!isAfter(currentDate, lastDate)) {
    ...
    const increment =
      ...
      netWorthOverTimeToFuture[firstDateOfPeriod];
...
const myMoneyInTheFuture = ({date, netWorthOverTimeToFuture}) =>
  netWorthOverTimeToFuture[formatDate(new Date(date))];

export const differenceInFuture = ({
  netWorthOverTimeToFuture = {},
  financialGoal,
}) =>
  myMoneyInTheFuture({
...
export const calculateTotalAveragePerDayOverTimeData = ({
  netWorthOverTimeToFuture,
}) => {
  const dates = Object.keys(netWorthOverTimeToFuture);
  const firstDate = dates[0];
...
export const getCanSpendAmount = ({
  netWorthOverTimeToFuture,
  netWorthOverTime,
  financialGoal,
  importantDates,
    ...
    (differenceInFuture({
      ...
      differenceInDays(
        ...
        importantDates[importantDates.length - 1] ||
          Object.keys(netWorthOverTime)[0],
      )) /
      differenceInDays(
        Object.keys(netWorthOverTimeToFuture).pop(),
        Object.keys(netWorthOverTime).pop(),
      ),
  );
...
export const getProjectedSavingForThisMonth = ({netWorthOverTime}) => {
  ...
  const projectedSavingForThisMonth = Math.floor(
    getDaysInMonth(lastDate) * lastOneMonthGrowthRate,
  );

  const daysLeft = differenceInDays(endOfMonth(lastDate), lastDate);
  const currentLevel =
    netWorthOverTime[formatDate(lastDate)] -
    netWorthOverTime[formatDate(endOfMonth(oneMonthAgo))];
  const artificialShortTermGrowthRate = daysLeft
    ? (projectedSavingForThisMonth - currentLevel) / daysLeft
    : 0;
  return {
...
export const determineStableIncome = ({netWorthOverTime}) => {
  ...
  if (
    ...
    return (
      (salaryCandidates[0] + salaryCandidates[1] + salaryCandidates[2]) / 3
    );
  } else {
    if (
      salaryCandidates[0] === salaryCandidates[1] ||
...
export const shouldPushTransactionsToLambda = async () => {
  ...
  if (connectionInfo.type !== 'none') {
    ...
    return response.status === 200
      ? response.data.body.syncStatus !== 'ok'
      : false;
  } else {
    return false;
  }
...
export const pushTransactionsToLambda = async ({accounts}) => {
  const response = await axios.post(
    'https://ucfcbyvt3h.execute-api.us-east-2.amazonaws.com/beta/transactions',
    {
      accounts,
    },
  );

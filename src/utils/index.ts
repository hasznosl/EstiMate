...
export const calculateProjectedNetWorthOverTime = ({
  ...
  const virtual =
    virtualSpending && !isNaN(parseInt(virtualSpending.value))
      ? parseInt(virtualSpending.value)
      : 0;
      ...
      if (!netWorthOverTime[currentDate] && isBefore(currentDate, new Date())) {
        ...
        projectedNetWorthOverTime[currentDate] =
          (projectedNetWorthOverTime[endOfThisMonth] ||
            netWorthOverTime[endOfThisMonth] || lastValue) +
...
export const calculateMonthlyAverageSpending = ({
  ...
  for (let i = 1; i < 32; i++) {
    const dates = Object.keys(worthChangeForEveryMonths).filter(
      date => getDate(date) === i
    );
